...
class TwitterWidet {
  constructor(commentId) {
    ...
    var commentTitle =
      '<ul class="breadcrumb twitter"><li class="active">Comments</li><li class="pull-right"></li></ul>';
    var twitterTemplate =
      ...
      window.location.href +
      "</a>";
    document.getElementById("twitter_frame_container").innerHTML =
      commentTitle + twitterTemplate;
  ...
  autoSizeTimeline() {
    if (this.$twitter.contentDocument) {
      var $hfeed = this.$twitter.contentDocument.getElementsByClassName(
        "timeline-TweetList"
      )[0];
      var $noMorePane = this.$twitter.contentDocument.getElementsByClassName(
        "timeline-LoadMore"
      )[0];
      var twitterht = 0;
      var add_ht = 0;
      if ($hfeed) {
        twitterht = Number(
          window.getComputedStyle($hfeed).height.replace("px", "")
        );
      }
      if ($noMorePane) {
        add_ht = Number(
          window.getComputedStyle($noMorePane).height.replace("px", "")
        );
      }
...
var CreateTitter = React.createClass({
  ...
  getInitialState: function() {
    ...
    return {
      ...
      isTemporary: false,
      addFundsMode: false,
      titterFrameUrl: getServiceUrl("titter") +
        "/iframe/?origin=" +
        origin +
        authorId,
  ...
  createListeners() {
    WindowActions.titterMessage.listen(msg => {
      if (msg.titterHeight) {
        document.getElementById("titteriframe").style.height =
          msg.titterHeight + "px";
      }
      if (msg.goAddFunds) {
    ...
    WindowActions.webGoldMessage.listen(msg => {
      if (msg.webgoldHeight) {
        document.getElementById("webgoldiframe").style.height =
          msg.webgoldHeight + "px";
      }
    });
  ...
  render: function() {
    ...
    if (!WrioDocument.hasCommentId() || this.state.nocomments) {
      ...
      parts.push(
        <CommentsDisabled key="comdis" isAuthor={this.state.isAuthor} />
      );
    ...
    if (!this.state.addFundsMode) {
      addCommentFundsMode = (
        <ul className="breadcrumb" key="act">
          ...
          <li style={{ display: "none" }}>
            <a onClick={this.switchToAddFundsMode}>Add funds</a>
          </li>
        </ul>
      );
      ...
      addCommentFundsMode = (
        <ul className="breadcrumb">
          <li>
            <a onClick={this.switchToAddCommentMode}>{this.state.addComment}</a>
          </li>
      ...
      parts.push(
        <iframe
          id="webgoldiframe"
          src={this.state.webgoldIframeUrl}
          style={this.editIframeStyles}
        />
      );
    }

    parts.push(<div id="twitter_frame_container" style={twStyle} />);
...
class CommentsDisabled extends React.Component {
  render() {
    ...
    var frameUrl =
      getServiceUrl("core") + "/edit?comment_article=" + window.location.href;
