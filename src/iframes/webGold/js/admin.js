...
class EthereumStats extends React.Component {
  ...
  getLatestBlockEtherscan() {
    let url =
      "https://ropsten.etherscan.io/api?module=proxy&action=eth_blockNumber&apikey=3854N5NEEKKCP4E4EB26W5SRG5D8ZSBGSK";
  ...
  render() {
    ...
    let master =
      "https://ropsten.etherscan.io/address/" + this.state.masterAddr;
    return (
      <div>
        <h1>WebGold admin</h1>
        ...
        <p>
          {" "}
          Master account:<a href={master}> {this.state.ethBalance} ETH </a>{" "}
        </p>
        <p> Master account: {this.state.wrgBalance} WRG </p>
        <p>
          {" "}
          Latest block {this.state.latest} (local) /{" "}
          {this.state.masterLatestBlock} (exact). If local block is lower than
          remote one, then there is some problem with local node, most likely it
          was stuck at some old block
        </p>
        <p> Gas price: {this.state.gasPrice} WRG </p>
        {sync}
        <p>
          {" "}
          Blockchain.info address gap: {this.state.gap} .{" "}
          <b>
            {" "}
            Warning! If gap > 20 we will be unable to generate new bitcoin
            incoming adresses!
          </b>
        </p>
      </div>
  ...
  getSyncBlock() {
    if (this.state.syncing) {
      ...
      return (
        <p>
          {" "}
          Sync in progress: #{this.state.currentBlock} / #{this.state.highestBlock}{" "}
        </p>
      );
    }
  }
        ...
        <table className="table">
          ...
          <tbody>
            {this.state.data.map(
              function(item) {
                var explorerLink =
                  "https://ropsten.etherscan.io/address/" + item.ethWallet;
                return (
                  <tr>
                    <td>{item.wrioID}</td>
                    <td>{item.name}</td>
                    <td>
                      <a href={explorerLink}>{item.ethWallet}</a>
                    </td>
                    <td>{item.ethBalance}</td>
                    <td onClick={this.showModal.bind(this, item.widgets)}>
                      {item.dbBalance}
                    </td>
        ...
        <Modal show={this.state.showModal} onHide={this.close.bind(this)}>
          ...
          <Modal.Body>
            {this.state.modalContent ? (
              <PrePayments data={this.state.modalContent} />
            ) : (
              ""
            )}
          </Modal.Body>
  ...
  render() {
    return (
      <div>
        ...
        <p>
          Description: to ensure proper user account operation each accound is
          feeded with minimal ether amount to perform opartion. Each ether
          withdrawal by user is logged in this page
        </p>
        <table className="table">
          <thead>
            <tr>
