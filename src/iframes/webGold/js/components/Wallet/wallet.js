  ...
  return (
    <div className="col-xs-12">
      <div className="pull-right">
        <a onClick={onReject} className="btn btn-default">
          <span className="glyphicon glyphicon-remove" />Cancel
        </a>
        <a href="#" className="btn btn-success" onClick={onApprove}>
          <span className="glyphicon glyphicon-ok" />Send
        </a>
      </div>
    </div>
  );
  ...
  sendSignedTransaction(tx) {
    ...
    sendSignedTransaction(tx, txId)
      ...
      .catch(err => {
        ...
        window.opener.postMessage(
          JSON.stringify({ closePopup: true, error: err }),
          "*"
        );
        setTimeout(window.close, 2000);
      });
  ...
  signTX(keystore) {
    ks
      ...
      .catch(err => {
        this.setState({
          error:
            "There seems to have been an error initializing your transaction."
        });
        console.log(err);
      });
  }
  ...
  checkCreds(keystore) {
    ks
      ...
      .then(result => {
        if (!result) {
          this.setState({
            error: "The seed you entered does not match your account."
          });
        } else {
          this.setState({
            approveStage: true,
        ...
        <div className="content col-xs-12">
          <div className="margin">
            <ul className="breadcrumb">
              <li className="active">Transaction is being processed</li>
            </ul>
      ...
      return (
        <div className="content col-xs-12">
          <div className="margin">
            ...
            <p className="col-xs-12">
              Transaction has been sent successfully. Transaction hash{" "}
