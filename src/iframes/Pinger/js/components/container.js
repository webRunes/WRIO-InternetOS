...
const BalancePane = ({ wrg, rtx, haveWallet, faucet }) => {
  return (
    <div className="well" id="balancePane">
      <h4 id="balancestuff">
        Current balance&nbsp;
...
const ErrorBox = ({ noAuthor, noAuthorWallet, noWebgold }) => {
  return (
    <div className="margin-bottom" role="form">
      {noWebgold && (
        <div id="nowebgold" className="callout warning col-xs-12">
          <h4>
            We are experiencing technical difficulties with payment service. We
            are working to fix the problem.
          </h4>
        </div>
      )}
      {noAuthor && (
        <div className="callout warning col-xs-12" id="noAuthor">
          <h5>Author unknown</h5>
          <p>You can comment, but donations disabled</p>
        </div>
      )}
      {noAuthorWallet && (
        <div className="callout warning col-xs-12" id="authorNoWallet">
...
const SendButton = ({ user, busy, sendCB, commentLeft }) => {
  ...
  const cb = user
    ? sendCB
    : () => {
        FormActions.openAuthPopup();
      };
  const btn = (
    <button
      type="button"
      className={`btn btn-primary ${busy ? "disabled" : ""}`}
      onClick={cb}
    >
      {busy ? <Loading /> : <span className="glyphicon glyphicon-ok" />}
      {text}
    </button>
  );
...
const ResultMessage = ({ donateResultText, error }) => {
  return (
    donateResultText && (
      <div
        className={"alert alert-success " + (!!error ? "danger" : "")}
        id="donatedStats"
      >
...
class Container extends React.Component {
  ...
  render() {
    return (
      <div id="titter-id">
        <ResultMessage
          donateResultText={this.state.donateResultText}
          error={this.state.donateError}
        />
        <form
          encType="multipart/form-data"
          method="POST"
          action="/sendComment"
          className="margin-bottom form-send-comment"
          role="form"
        >
          {this.state.showBalance && (
            <BalancePane
              wrg={this.state.balance}
              rtx={this.state.rtx}
