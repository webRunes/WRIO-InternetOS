  ...
  render() {
    ...
    return (
      <div className="dashboard-main">
          ...
          <div className="dashboard-modal-div">
            <form>
              <div className="callout warning col-xs-12">
                <h5>The functionality is available to Alpha testers only.</h5>
              </div>
              <p>
                Press "Connect" on the WRIO IoT Gateway to register a new device, or enter the IPv6
                address and AES key manually. For more information check manual inside the box.
              </p>
        ...
        <div className="callout warning col-xs-12">
          <h5>
            Dasboard is under development and the functionality is limited. Premium features are
            available to Alpha testers only.
          </h5>
        </div>
        ...
        <div className="row">
          <div className="control-left control-panel col-sm-6">
            ...
            <p>
              8<sup>
                +2<span className="glyphicon glyphicon-arrow-up" />
              </sup>
            </p>
            <Tooltip className="tooltip-b" content="Available to Alpha testers only">
          ...
          <div className="control-right control-panel col-sm-6">
            ...
            <Tooltip
              className="tooltip-b"
              content="Premium feature, available to Alpha testers only"
            >
        ...
        <div className="row">
          <div className="col-xs-12">
            <ul className="nav nav-pills tab_networks">
              <li role="presentation" className={this.state.isActiveOne ? 'active' : ''}>
                <a
                  href=""
                  data-toggle="tab"
                  onClick={(e) => {
              ...
              <li role="presentation" className={this.state.isActiveTwo ? 'active' : ''}>
                <a
                  href=""
                  data-toggle="tab"
                  onClick={(e) => {
              ...
              <li role="presentation" className={this.state.isActiveThree ? 'active' : ''}>
                <a
                  href=""
                  data-toggle="tab"
                  className="add_icon"
            ...
            <div className="row">
              <div className="col-xs-6">
                <button type="button" className="btn btn-success" onClick={() => this.openModal()}>
                  <span className="glyphicon glyphicon-plus with_text" />Add New Device
                </button>
              </div>
              <div className="col-xs-6 search">
                <input
                  ...
                  data-toggle="tooltip"
                  data-placement="top"
                  title="Available to Alpha testers only"
                />
              </div>
            </div>
                ...
                <tbody>
                  {sensorData.length > 0
                    ? sensorData.map((sensor, index) => {
                        ...
                        let temperatureData = sensor.payload.dataFeedElement.find(item => item.item.variableMeasured.name.toLowerCase() == 'temperature');
                        let batteryData = sensor.payload.dataFeedElement.find(item => item.item.variableMeasured.name.toLowerCase() == 'battery');
                        let stateData = sensor.payload.dataFeedElement.find(item => item.item.variableMeasured.name.toLowerCase() == 'state');
                        let lastIndex = sensorDateModified.length - 1;
                        let dropDownItems = sensor.payload.dataFeedElement.filter(item =>
                            item.item.variableMeasured.name.toLowerCase() != 'battery' &&
                            item.item.variableMeasured.name.toLowerCase() != 'state');
                        let datadesc = dropDownItems
                          ? dropDownItems.map((item, index) =>
                                (item.item.variableMeasured.name == 'temperature'
                                  ? `${item.item.variableMeasured.name[0].toUpperCase() +
                                      item.item.variableMeasured.name.slice(1)}: ${
                        ...
                        let datadesclist = datadesc
                          .toString()
                          .split(',')
                          .join('  \n ');
                        let checkEnable = stateData.item.variableMeasured.value[
                          lastIndex
                        ].toLowerCase();
                        ...
                        return (
                          ...
                          >
                            <td>
                              <Tooltip content={sensorProductData.productID}>
                                <div className="dashboard-sensor-id">
                                  {sensorProductData.productID}
                                </div>
                              </Tooltip>
                            </td>
                            <td>
                              <a href={sensor.url.substring(0, sensor.url.length - 5)}>
                                {sensorProductData.name}
                              </a>
                            </td>
                            <td className="center">
                              {stateData.item.variableMeasured.value[lastIndex].toLowerCase() ==
                              'enable' ? (
                                <Tooltip content="Enabled">
                                  <span className="glyphicon glyphicon-ok-sign icon-success" />
                                </Tooltip>
                              ) : (
            ...
            <div className="row">
              <div className="col-sm-12">
                <div className="jumbotron">
                  ...
                  <p>
                    This is a private IoT network. You need to gain access rights from the network
                    provider.
                  </p>
                  <p>
                    <Tooltip className="tooltip-b" content="Available to Alpha testers only">
                      <button className="btn disabled">Get access</button>
                    </Tooltip>
                  </p>
                </div>
              </div>
            ...
            <div className="row">
              <div className="col-sm-12">
                <div className="jumbotron">
                  ...
                  <p>
                    You are not a member of the provider's network. To get access you need to join
                    the network.
                  </p>
                  <p>
                    <Tooltip className="tooltip-b" content="Available to Alpha testers only">
                      <button className="btn disabled">Join the network</button>
                    </Tooltip>
                  </p>
                </div>
              </div>
