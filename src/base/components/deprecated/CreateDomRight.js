...
class ArticleButton extends MenuButton {
  onClick(e) {
    ...
    WrioDocumentActions.article(
      this.props.data.name,
      replaceSpaces(this.props.data.name)
    );
    super.onClick(e);
  }
}
...
var CreateDomRight = React.createClass({
  ...
  componentDidMount: function() {
    ...
    this.listenStoreMenuWindowResize = StoreMenu.listenTo(
      ActionMenu.windowResize,
      this.onWindowResize
    );
  },
  ...
  onWindowResize: function(width, height) {
    if (width > 767) {
      if (height < ReactDOM.findDOMNode(this.refs.sidebar).offsetHeight) {
        this.setState({
          resize: true
        });
      }
      ...
      <div className={className} id="sidebar">
        <div ref="sidebar" className="sidebar-margin">
          ...
          {articleItems.length > 0 ? (
            <nav className="contents visible-md-block visible-lg-block">
              {" "}
              {/* add "navbar-fixed-top" and id="cd-vertical-nav" for small displays */}
              <h1>Contents</h1>
              <ul style={height}>
                {articleItems}
                <ArticleButton
                  data={{ name: "Comments", url: "#Comments" }}
                  active={this.active}
          ...
          {externalItems.length > 0 ? (
            <ul style={height}>{externalItems}</ul>
          ) : (
            ""
          )}
        </div>
