...
class CreateDomLeft extends React.Component {
  ...
  componentDidMount() {
    this.listenStoreMenuToggle = StoreMenu.listenTo(
      ActionMenu.toggleMenu,
      this.toggleMenu
    );
    this.listenStoreMenuSidebar = StoreMenu.listenTo(
      ActionMenu.showSidebar,
      this.showSidebar
    );
    this.listenActionsResize = ActionMenu.windowResize.listen(
      this.windowResize
    );
  }
  ...
  toggleMenuByClick() {
    this.setState({
      toggleMenu: !CreateDomLeft.hasClass(
        ReactDOM.findDOMNode(this.refs.toggleMenu),
        "active"
      )
    });
    ActionMenu.toggleMenu(
      !CreateDomLeft.hasClass(
        ReactDOM.findDOMNode(this.refs.toggleMenu),
        "active"
      ),
      !CreateDomLeft.hasClass(
  ...
  showSidebarByClick() {
    this.setState({
      showSidebar: !CreateDomLeft.hasClass(
        ReactDOM.findDOMNode(this.refs.showSidebar),
        "active"
      )
    });
  ...
  windowResize(width, height) {
    if (
      this.refs.navbarHeader &&
      window.innerHeight <
        ReactDOM.findDOMNode(this.refs.navbarHeader).offsetHeight +
          height +
          41 &&
      ...
      var pht =
        window.innerHeight -
        (ReactDOM.findDOMNode(this.refs.navbarHeader).offsetHeight + 41);
  ...
  render() {
    ...
    return (
      <div className="col-xs-12 col-sm-3 col-md-2">
        <div
          ref="navbar"
          className="navbar navbar-inverse main navbar-fixed-top row-offcanvas-menu"
        >
          {header}
          {this.props.list}
        </div>
  ...
  getHeader(classNameToggle, classNameSidebar) {
    return (
      <div
        ref="navbarHeader"
        className="navbar-header tooltip-demo"
        id="topMenu 12"
      >
        <ul className="nav menu pull-right">
          <li
            ...
            data-toggle="tooltip"
            data-original-title="Open/close menu"
          >
            <a
              onClick={this.toggleMenuByClick}
              ref="toggleMenu"
          ...
          <li
            ...
            data-placement="bottom"
            data-toggle="tooltip"
            data-original-title="Show/hide the sidebar"
          >
            <a
              onClick={this.showSidebarByClick}
              ref="showSidebar"
