...
class MapBox extends React.Component {
  ...
  static mapOnClick(cordinates, description,sensorProductName,checkEnable) {// for popup
    let filteredGeoCoordinates = [cordinates[0].longitude, cordinates[0].latitude];
    let Description = description;
    let ProductName = sensorProductName;
    let el = document.createElement('tr');
    el.id = 'marker';
    Description = Description.toString().split(",").join(" <br /> ");
    let text;
    if(checkEnable == "enable"){
      text = '<div style="background:#99ff99; color:white">'+'<p>'+ProductName+'</p>'+"<div style={{whiteSpace: 'pre-line'}}><p>"+Description+'</p></div>'+'<p>'+filteredGeoCoordinates+'</p>+</div>';
    }else{
      text = '<div style="background:#8585ad; color:white">'+'<p>'+ProductName+'</p>'+"<div style={{whiteSpace: 'pre-line'}}><p>"+Description+'</p></div>'+'<p>'+filteredGeoCoordinates+'</p>+</div>';
    }
    const mapBoxGL = window.mapboxgl || undefined;
    if (mapBoxGL) {
      mapboxgl.accessToken = Token;
