export default class TwitterTimelineWidget {
  constructor(commentId, container) {
    ...
    var commentTitle =
      '<ul class="breadcrumb twitter"><li class="active">Comments</li></ul>';
    var twitterTemplate =
  ...
  onTimelineLoad() {
    ...
    this.$twitter.contentDocument.getElementsByTagName(
      "style"
    )[0].innerHTML += `
            img.autosized-media {
              width:auto;
  ...
  autoSizeTimeline() {
    if (this.$twitter.contentDocument) {
      const getHeight = $el =>
        !!$el
          ? Number(window.getComputedStyle($el).height.replace("px", ""))
          : 0;
      const getElm = name =>
        this.$twitter.contentDocument.getElementsByClassName(name)[0];
