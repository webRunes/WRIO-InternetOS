    ...
    return sortBy(
      ...
      return (
        <Item
          ...
          del={del}
          data={i}
          key={"sub" + encodeURIComponent(i.url)}
          child={true}
        />
      );
    });
  ...
  render() {
    const data = this.props.data,
      ...
      rightContent = children ? (
        ...
        <span onClick={this.del} className="glyphicon glyphicon-remove" />
      ),
    ...
    return (
      <li className={className}>
        <a
          ...
          className={!haveActiveChildren ? "collapsed active" : "collapsed"}
        ...
        <div className="in" style={childrenStyle(shouldOpen)}>
          <ul className="nav nav-pills nav-stacked sub">{list}</ul>
        </div>
      </li>
    );
  }
