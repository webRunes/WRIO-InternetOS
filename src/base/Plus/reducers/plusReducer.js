...
function checkHaveEverything(state) {
  if ((state.authorData || state.noAuthor) && state.plusData) {
    const _tabs = importPlusState(
      state.plusData,
      !isPlusActive(state.wrioID),
      state.authorData
    ); // don't add current page to tabs if plus active
...
async function onDel(listName, elName) {
  const [newdata, next, wasActive] = deletePageFromTabs(
    this.data,
    listName,
    elName
  );
...
async function onClickLink(data) {
  ...
  if (window.localStorage) {
    localStorage.setItem(
      "tabScrollPosition",
      document.getElementById("tabScrollPosition").scrollTop
    );
  }
